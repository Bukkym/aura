...
### Slice 6 — Vercel deploy (ships before Module 3, final slice) — scaffolding ready, deploy pending

Code/config scaffolding is in place: `vercel.json` (region `fra1`), `technical/07-deployment.md` (the click-by-click runbook), `.env.local.example` annotated for runtime-vs-seed-vs-migration, and the runtime-env audit confirming only the three supabase-js vars are needed (no `OPENAI_API_KEY` in prod). The remaining work is dashboard actions (Vercel project + domain, Supabase redirect URL, Resend SMTP) + a manual magic-link sign-in smoke, which need the operator's accounts. Follow `technical/07-deployment.md`.
