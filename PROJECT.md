...
Sequenced into slices, executed one at a time. AI-dependent slices are on hold until `OPENAI_API_KEY` is added back to the Vercel runtime env (removed in Slice 6).
...
- **M4.2 LLM-narrated "why this plan."** `lib/whyNarrate.ts` wraps an LLM call on the same payload `whyTemplates.ts` uses; falls through to the deterministic template on error or >800ms timeout. Needs runtime OpenAI.
- **M4.3 Voice onboarding (real).** MediaRecorder capture, Whisper transcription, `lib/extract.ts` returning JSON-schema-constrained output mapped to the chip taxonomy, pre-filling onboarding chips. Chip schema stays the contract. Needs runtime OpenAI.
- **M4.4 Lazy real embeddings** for chip-onboarded users (background re-embed, upgrades tiebreak quality; no user-visible change). Needs runtime OpenAI; runtime embeddings must use `text-embedding-3-small` to match seed vectors.
- **M4.5 Live stretch moment.** The "Ora has learned this about you" beat on real data. Depends on M4.1 history + M4.2 narration.
